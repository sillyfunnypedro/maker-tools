...
/**
 * Remove redundant pixels left over by thinning.
 *
 * Zhang-Suen does not always leave a strictly 1-pixel line: along a diagonal it
 * leaves 2-pixel-wide staircase steps ("##" pairs). Those doubled pixels are
 * harmless to look at but poison the skeleton *graph* — the connectivity test in
 * the tracer counts them as extra neighbour groups, reads them as junctions, and
 * splits one continuous curve into fragments plus tiny duplicate stubs (the two
 * ways around each pair). A traced circle came out as several open arcs instead
 * of one closed loop because of this.
 *
 * A pixel is redundant when its ON neighbours are all still connected to one
 * another without it — then deleting it cannot break the line. Note that this is
 * *not* the same as the neighbours forming one unbroken run around the ring: on a
 * staircase the neighbours split into two ring-runs that are nevertheless
 * diagonally adjacent to each other, so the ring test rejects exactly the pixels
 * we need to remove. Testing adjacency between the neighbours directly is what
 * actually answers the question.
 *
 * Pixels with fewer than two neighbours are endpoints and are kept, so lines are
 * never shortened away. A straight run is never touched either: its two
 * neighbours sit on opposite sides, two cells apart, so they are not adjacent.
 */
function thinRedundant(skel: Uint8Array, w: number, h: number): void {
  // ring offsets and their (dx, dy), clockwise from top-left
  const dx = [-1, 0, 1, 1, 1, 0, -1, -1];
  const dy = [-1, -1, -1, 0, 1, 1, 1, 0];
  const ring = dx.map((_, k) => dy[k] * w + dx[k]);
  const on: number[] = [];
  const seen = new Uint8Array(8);
  const queue = new Int32Array(8);
  ...
  while (changed && guard++ < 20) {
    ...
    for (let y = 1; y < h - 1; y++) {
      for (let x = 1; x < w - 1; x++) {
        ...
        if (!skel[i]) continue;
        on.length = 0;
        for (let k = 0; k < 8; k++) if (skel[i + ring[k]]) on.push(k);
        if (on.length < 2) continue; // endpoint
        // Flood the neighbour set using Chebyshev adjacency between neighbours.
        seen.fill(0);
        let qn = 0, qh = 0, reached = 1;
        seen[0] = 1;
        queue[qn++] = on[0];
        while (qh < qn) {
          const a = queue[qh++];
...
export function computeMasks(
  ...
  thinRedundant(skeleton, w, h); // collapse 2px staircase steps before tracing
